...
|              | veth pair                                                            |
|              v                                                                      |
|  [caliXXX] -> [IPVS/iptables DNAT: 10.96.0.100 -> 10.244.2.3] -> [BGP route] -> [eth0]
+------------------------------------------------------------------------------------+
                                   |
                                   | 物理网络（BGP 模式：裸 IP 包）
                                   v
+------------------------------------------------------------------------------------+
| node2 (192.168.1.11)                                                               |
|  [eth0] -> [route: 10.244.2.3 dev caliYYY] -> [caliYYY]                            |
|                                     | veth pair                                     |
|                                     v                                               |
|                      +------------------------------+                               |
|                      | PodB 网络命名空间             |                               |
...
#### ② **独立使用示例（NAT 模式：一台 LVS 机器负载均衡三台后端）**（ASCII 图）
...
[LVS VIP 10.0.0.1]
      |
      | ② DNAT (rr) -> RS1 10.0.0.10:80
      v
[RS1 10.0.0.10:80]   (备选: RS2 10.0.0.11, RS3 10.0.0.12)
      |
      | ③ 响应返回 LVS
      v
[LVS 做反向 SNAT: src 10.0.0.10 -> 10.0.0.1]
      |
      | ④ 客户端看到来自 VIP
      v
[Client 1.2.3.4]
...
```mermaid
graph LR
    subgraph NETFILTER_LOCAL["本机内核 netfilter"]
...
#### ② **独立使用示例（端口转发：本机 8080 → 后端 192.168.1.100:80）**（ASCII 图）

```text
[网卡接收 dst=本机IP:8080]
        |
        v
[PREROUTING(nat)] --DNAT--> [dst=192.168.1.100:80]
        |
        v
[FORWARD(filter)] --> [POSTROUTING(nat)] --MASQ--> [发往后端]

[后端响应 src=192.168.1.100]
        |
        | conntrack 自动反向 NAT
        v
[PREROUTING] -> [FORWARD] -> [POSTROUTING] -> [返回原始客户端]
```
...
```mermaid
graph TD
    subgraph VXLAN_NODE1["node1 (192.168.1.10)"]
        APP1["应用: 10.1.0.1"]
        VTEP1["VTEP<br/>vxlan100<br/>绑定 192.168.1.10"]
        ETH1["eth0<br/>192.168.1.10"]
    end

    subgraph VXLAN_NODE2["node2 (192.168.1.11)"]
        ETH2["eth0<br/>192.168.1.11"]
...
#### ② **独立使用示例（两台机器手动建立 VXLAN 隧道）**（ASCII 图）

```text
node1 (192.168.1.10)                                node2 (192.168.1.11)
+----------------------------------------+          +----------------------------------------+
| [APP1 10.1.0.1]                        |          |                        [APP2 10.1.0.2] |
|        |                               |          |                               ^        |
|        v                               |          |                               |        |
| [VTEP vxlan100] -> [eth0] ======================> [eth0] -> [VTEP vxlan100]      |        |
|   封装: outer 192.168.1.10->192.168.1.11        |   物理网络仅看到 UDP:4789      |        |
|        inner 10.1.0.1->10.1.0.2                 |   解封装后还原内层包            |        |
+----------------------------------------+          +----------------------------------------+
```
...
```mermaid
graph LR
    subgraph HOST_NS_DEFAULT["host network namespace（默认）"]
        H["host 进程<br/>能访问物理网卡<br/>也能操作 ns-a/ns-b"]
    end

    subgraph VETH_NS_A["ns-a"]
        VA["veth0<br/>10.0.0.1/24"]
    end

    subgraph VETH_NS_B["ns-b"]
...
#### ② **独立使用示例（手动创建两个 ns，用 veth pair 连通）**（ASCII 图）
...
[host network namespace]
        ...
        | 管理 ns-a / ns-b
        v
[ns-a] veth0 10.0.0.1/24  <====== 内核内存直连(veth pair) ======>  veth1 10.0.0.2/24 [ns-b]
```
...
graph TD
    ...
    CALC["Felix Calculation Layer<br/>策略计算引擎<br/>将高层意图转换为底层规则"]
    DP["Felix Dataplane Driver"]

    subgraph FELIX_KERNEL_DP["内核数据平面"]
...
[k8s API Server]
       ...
       | Watch 变化事件
       v
[Felix Calculation Layer]
       |
       | 计算增量规则
       v
[Felix Dataplane Driver]
   |                |                 |
   v                v                 v
[iptables]      [主机路由表]      [eBPF 程序]
```

**步骤说明：**

1. Felix 启动后 Watch k8s API Server，监听 NetworkPolicy、Pod、Node 的增删改
2. 收到变化事件，Calculation 层计算出受影响的规则集（增量计算，不是全量重建）
...
```mermaid
graph TD
    subgraph ADDR_PHY_VM["物理/虚拟机网络层"]
    ...
    subgraph ADDR_POD_NET["Pod 网络层（Calico 管理）"]
        POD_A["PodA<br/>veth: 10.244.1.2"]
        POD_B["PodB<br/>veth: 10.244.2.3"]
    end

    subgraph ADDR_SVC_VIP["Service 虚拟层（IPVS/iptables 管理）"]
...
```text
物理/虚拟机网络层:
[node1 eth0 192.168.1.10] --- [交换机/路由器] --- [node2 eth0 192.168.1.11]

Pod 网络层 (Calico):
[PodA 10.244.1.2] -------------------------------> [PodB 10.244.2.3]
          (经 BGP/VXLAN 路由可达)

Service 虚拟层 (IPVS/iptables):
[Service 10.96.0.100 (无真实网卡)]
    |
    | DNAT 映射
    v
[PodB 10.244.2.3]
...
# Pod CIDR（给 Calico 用）: 10.244.0.0/16
# Service CIDR:            10.96.0.0/12
